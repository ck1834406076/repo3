...
<div id="w0" align="center">
   ...
   <a type="button"  href="${pageContext.servletContext.contextPath}/create.jsp">添加学生</a>
    </div>
    <div >
        <form action="${pageContext.servletContext.contextPath}/student/query" method="post">
            <div class="btn-group">
                    ...
                    <tr>
                        ...
                        <c:if test="${not empty gradelist}">
                            <c:forEach items="${gradelist}" var="grade">
                                <c:if test="${stu.gradeid==grade.gradeId}">
                                    ${grade.gradename}
                                </c:if>
                            </c:forEach>
                        </c:if>
...
<script>
    function deleteById(id){
        location.href="${pageContext.servletContext.contextPath}/student/delete?id="+id;
    }
    function updateStu(id) {
        location.href="${pageContext.servletContext.contextPath}/student/selectById?id="+id;
    }
    function prePage(page) {
        if(${pageInfo.hasPreviousPage}){
            location.href="${pageContext.servletContext.contextPath}/student/query?pageNo="+(page-1)+"&sname="+$("[name=sname]").val();
        }else {
            alert("已经是第一页了")
        }
    }
    ...
    function go(page) {
        if(page>=1&&page<=${pageInfo.pages}&&page%1==0){
            location.href="${pageContext.servletContext.contextPath}/student/query?pageNo="+(page)+"&sname="+$("[name=sname]").val();
        }else {
            alert("请输入有效的页码")
        }
    }
